# 任务摘要：Exercise Library 官方种子版本化 + 增量更新 API（FastAPI）

- 日期：2026-01-02
- 相关架构说明：`docs/PROJECT_OVERVIEW.md`
- 背景：Android 端已实现“官方种子导入 + 版本 SSOT + 增量更新（失败回退全量）”，并采用 **WorkManager 每日一次拉取检测**（非 SSE 常驻推送）。

## 🎯 目标（WHY）

- 让服务端可以“发布/更新官方动作库”，客户端在不频繁请求的前提下（1 天 1 次）自动同步到最新。
- 官方库与用户自定义库物理/逻辑隔离：官方发布不会污染用户自定义数据。

## ✅ 客户端已固定的契约（必须匹配）

> 路径前缀：`/api/v1/exercise/library/*`  
> 响应：直出 JSON，不包裹 `code/data` 二层结构；字段 camelCase。

1) `GET /api/v1/exercise/library/meta`
- 返回：`{ "version": <int>, "totalCount": <int>, "lastUpdated": <long(ms)>, "checksum": <string>, "downloadUrl": <string|null> }`
- 语义：服务端当前“官方库快照”的最新版本信息（version 单调递增）。

2) `GET /api/v1/exercise/library/full`
- 返回：`List[ExerciseDto]`（与 App 侧 `ExerciseDto` 字段对齐）
- 语义：返回最新版本的完整官方库（用于客户端回退全量覆盖）。

3) `GET /api/v1/exercise/library/updates?from=<int>&to=<int>`
- 返回：`{ "fromVersion": <int>, "toVersion": <int>, "added": List[ExerciseDto], "updated": List[ExerciseDto], "deleted": List[string], "timestamp": <long(ms)> }`
- 语义：从 `from` 到 `to` 的增量 diff（客户端会按 `id` 删除/批量 upsert）。
- 兼容：若 `from` 过旧或 diff 不可得，可返回 4xx/5xx 触发客户端自动回退 `full`。

## 🧱 数据/版本 SSOT（建议实现）

### 1) 官方库发布 SSOT
- 建议引入“官方库版本表”（snapshot 元信息），至少包含：
  - `version`（int，主键/唯一，单调递增）
  - `checksum`（对 canonical JSON 的 hash，用于一致性校验/缓存）
  - `generated_at`、`total_count`
  - `payload_location`（可选：文件路径/对象存储 key）

### 2) diff 生成规则（SSOT=同一套算法）
- Key：`ExerciseDto.id` 作为主键。
- 判定：
  - `added`：新版本存在、旧版本不存在
  - `deleted`：旧版本存在、新版本不存在
  - `updated`：两版本均存在但字段内容不同（建议对 canonical JSON 做 hash 对比）
- 注意：客户端只关心 id + 字段最终值；不需要复杂 merge 语义。

## 📦 “发布新种子”的入口（管理端/脚本）

> 本任务不强制 UI，但必须具备可回放的发布手段（脚本/管理 API 二选一）。

建议二选一：
1) **脚本发布（优先 KISS）**：提供 `scripts/publish_exercise_seed.py`（读 seed.json → 生成 version+checksum+diff → 写入 DB/文件）。
2) **管理 API 发布**（需要鉴权）：
   - `POST /api/v1/admin/exercise/library/publish`：上传 seed JSON（已实现）
   - 返回新 `meta`（version/checksum/totalCount）

实现约定（现状 SSOT）：
- 鉴权：仅 `admin` 用户可用（Dashboard 本地账号 / JWT `user_metadata.is_admin=true`）。
- 请求头：支持 `token: <jwt>` 或 `Authorization: Bearer <jwt>`。
- 请求体：支持直接传 `List[ExerciseDto]` 数组，或 `{ "items": [...] }` / `{ "payload": [...] }` / `{ "exercises": [...] }`。

## 🔐 鉴权与策略

- 客户端侧该接口属于 App 基础能力，建议对匿名用户也可读（与现有 PolicyGate 策略保持一致），但应走限流。
- 发布入口必须要求永久用户/管理员权限。

## 🧪 验证与验收（DONE）

1) 冒烟
- `GET /api/v1/exercise/library/meta` 返回 `version >= 1`
- `GET /api/v1/exercise/library/full` 返回非空列表，字段可被 App 解析
- `GET /api/v1/exercise/library/updates?from=0&to=<meta.version>` 可用或明确失败（触发客户端回退 full）

2) 合约测试（推荐）
- 新增 `tests/test_exercise_library_contracts.py`：
  - meta/full/updates 的 schema 校验（字段、类型、列表结构）
  - diff 规则：added/updated/deleted 不相交；deleted 只包含字符串 id

## 📝 备注（客户端侧实现提示）

- official id：当前官方 seed 使用 `off_${md5(name).take(8)}`（UTF-8，取 8 位十六进制小写）；更推荐直接以 seed 中的 `id` 为准，避免多语言实现漂移。
- App 侧每天只检测一次（节流 SSOT=本地 prefs 的 `last_check_at_ms`），不依赖 SSE 推送。
