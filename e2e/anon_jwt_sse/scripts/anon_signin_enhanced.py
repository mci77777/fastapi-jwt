...
class EnhancedAnonAuth:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(
                    edge_url, headers=headers, json={}, timeout=aiohttp.ClientTimeout(total=30)
                ) as response:
    ...
    async def get_token_via_native_auth(self, trace_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.post(
                    auth_url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=30)
                ) as response:
                    response_text = await response.text()
    ...
    async def verify_token_with_api(self, token: str) -> Dict[str, Any]:
        ...
        # 测试多个端点
        test_endpoints = [f"{self.api_base}/api/v1/me", f"{self.api_base}/api/v1/messages", f"{self.api_base}/health"]
        ...
        with open(verify_file, "w", encoding="utf-8") as f:
            json.dump(
                {"timestamp": datetime.now(timezone.utc).isoformat(), "api_base": self.api_base, "results": results},
                f,
                indent=2,
                ensure_ascii=False,
            )
...
async def main():
    ...
    parser.add_argument("--verify", action="store_true", help="验证获取的token是否能访问API")
