...
class AnonymousE2E:
    ...
    @staticmethod
    def _extract_otp_from_confirm_url(url: str) -> Optional[str]:
        """
        Supabase 的确认链接可能把 OTP 放在 query 的 token 参数中（邮件内容只展示链接）。
        若 token 是 6 位数字，则视为 OTP。
        """
        try:
            q = parse_qs(urlparse(url).query)
            token = (q.get("token") or q.get("otp") or q.get("code") or [None])[0]
    ...
    async def _send_email_otp(self, client: httpx.AsyncClient, email: str) -> StepRecord:
        """
        发送 Email OTP（与 Magic Link 共用实现）。
        GoTrue: POST /auth/v1/otp
        """
        ...
        # 成功时通常是 200/201/204 且 body 为空对象
        success = resp.status_code in (200, 201, 202, 204)
        return StepRecord(
            name="supabase_send_otp",
            success=success,
    ...
    async def _verify_email_otp(self, client: httpx.AsyncClient, email: str, otp: str) -> tuple[StepRecord, Optional[str]]:
        """
        校验 Email OTP 并换取 session（access_token）。
        Supabase client: verifyOtp({ email, token, type: 'email' })
        """
        url = f"{self.supabase_url}/auth/v1/verify"
        payload = {"email": email, "token": otp, "type": "email"}
        headers = {
            "apikey": self.supabase_anon_key or self.supabase_service_key,
            "Authorization": f"Bearer {self.supabase_anon_key or self.supabase_service_key}",
            "Content-Type": "application/json",
        ...
        step = StepRecord(
            name="supabase_verify_otp",
            success=resp.status_code == 200 and isinstance(token, str) and bool(token),
            request={"url": url, "payload": {"email": email, "token_len": len(otp), "type": "email"}},
    ...
    async def _wait_for_mail_otp(
        ...
        *,
        ...
        if not self.mail_api:
            step = StepRecord(name="mail_wait_otp", success=False, notes={"error": "mail_api_not_configured"})
            return step, None
                ...
                try:
                    ...
                    for m in messages:
                        ...
                        if not otp:
                            ...
                            for u in urls:
                                if "/auth/v1/verify" in u:
                                    otp = self._extract_otp_from_confirm_url(u)
                                    if otp:
                                        break
                        ...
                        if otp:
                            if otp_output_path:
                                try:
                                    Path(otp_output_path).write_text(
            ...
            while time.monotonic() < deadline:
                try:
                    ...
                    for m in messages:
                        ...
                        if confirm_url:
                            url_otp = self._extract_otp_from_confirm_url(confirm_url)
                            if otp_output_path and (not otp) and url_otp:
                                otp = url_otp
                                try:
                                    Path(otp_output_path).write_text(
                                        f"email={mailbox.email}\notp={otp}\n", encoding="utf-8"
                            ...
                            # confirm_method:
                            # - otp: 优先用 OTP 调 /auth/v1/verify（若无 OTP 再走 link）
                            # - link: 总是走 link
                            # - auto: 默认走 link（与 Supabase 邮件模板一致），无 link 时用 OTP fallback
                            otp_verify_status = None
                            if confirm_method == "otp" and otp:
                                # 先尝试 OTP 验证；若失败则回退到 link（避免 OTP 与 link 语义不一致导致整条链路失败）
                                ...
                                otp_verify_status = rr.status_code
                                if rr.status_code in (200, 201, 204):
                                    ...
                                    return StepRecord(
                                        ...
                                        response={"status_code": otp_verify_status},
                                        notes={
                                            "method": "otp",
                                            "otp_len": len(otp),
                                            "otp_output_path": otp_output_path,
                            ...
                            if confirm_method in ("auto", "link", "otp"):
    ...
    async def run(
        ...
        *,
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            if auth_method == "otp":
                if not mailbox:
                    print("ERROR: auth_method=otp 需要 email_mode=mailapi（用于收验证码）")
                    return await self._finalize(1)

                # 发送 OTP（可能触发自动注册）
                send_otp_step = await self._send_email_otp(client, email)
                self.report.add_step(send_otp_step)
                print(f"[STEP] Send OTP status: {send_otp_step.response['status_code']}")
                if not send_otp_step.success:
                    return await self._finalize(1)

                # 等待邮件验证码
                wait_step, otp = await self._wait_for_mail_otp(
                    mailbox=mailbox,
                    max_wait_seconds=mail_api_max_wait_seconds,
                    poll_interval_seconds=mail_api_poll_interval_seconds,
                    otp_output_path=otp_output_path,
                )
                self.report.add_step(wait_step)
                if not wait_step.success or not otp:
                    return await self._finalize(1)

                # 验证 OTP，换取 access_token
                verify_step, token = await self._verify_email_otp(client, email, otp)
                self.report.add_step(verify_step)
                print(f"[STEP] Verify OTP status: {verify_step.response['status_code']}")
                if not verify_step.success or not token:
                    return await self._finalize(1)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--confirm-method",
        choices=["auto", "link", "otp"],
        default=os.getenv("MAIL_CONFIRM_METHOD", "auto"),
        help="邮箱确认方式：auto=默认走链接（无链接再用OTP）；link=强制走链接；otp=优先走OTP（若无OTP再走链接）",
    )
    parser.add_argument(
        "--auth-method",
        choices=["password", "otp"],
        default=os.getenv("E2E_AUTH_METHOD", "password"),
        help="认证方式：password=signup+password grant（默认）；otp=发送验证码并 verifyOtp 登录（需要 Mail API 收码）",
    )
    parser.add_argument(
        "--print-mail-config",
        action="store_true",
...
async def async_main() -> int:
    ...
    return await runner.run(
        ...
        confirm_method=args.confirm_method,
