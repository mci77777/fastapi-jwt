...
class SSEEvent:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "timestamp_readable": time.strftime("%Y-%m-%d %H:%M:%S.%f UTC", time.gmtime(self.timestamp))[:-3],  # 保留毫秒
        }
    ...
    async def create_message(self, text: str, conversation_id: Optional[str] = None, skip_prompt: bool = False) -> str:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{self.api_base}/api/v1/messages", headers=headers, json=payload, timeout=30.0
            )
                ...
                async for line in response.aiter_lines():
                    ...
                    if not line:
                        ...
                        if current_event and current_data:
                            event = SSEEvent(event_type=current_event, data="\n".join(current_data))
