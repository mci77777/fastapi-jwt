...
class DockerDeploymentVerifier:
    ...
    def verify_docker_installed(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print("  [FAIL] Docker not working properly")
                self.checks_failed += 1
                return False
    ...
    def verify_web_build_config(self) -> bool:
        ...
        try:
            ...
            if "build" in scripts:
                ...
                print("  [FAIL] Build script missing")
                self.checks_failed += 1
    ...
    def print_summary(self):
        ...
        if self.checks_failed == 0:
            ...
            print("    docker build -t gymbro-api:latest .")
            print("\n  To run the container, run:")
            print("    docker run -d -p 9999:80 --name gymbro-api gymbro-api:latest")
            return 0
        else:
            print(f"\n  [WARNING] {self.checks_failed} check(s) failed")
            print("  Please fix the issues before building Docker image.")
            return 1
