...
class GWAuthVerifier:
    ...
    async def test_health_endpoints(self) -> bool:
        ...
        endpoints = ["/api/v1/healthz", "/api/v1/livez", "/api/v1/readyz"]
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            for endpoint in endpoints:
                try:
                    ...
                    all_passed = False
                    self.add_result(f"健康探针 {endpoint}", False, {"error": str(e)})
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code != 200:
                    self.add_result("Prometheus指标端点", False, {"status_code": response.status_code})
                    return False
            ...
            self.add_result("Prometheus指标端点", False, {"error": str(e)})
            return False
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                self.add_result(
                    "白名单路径免限流",
                    passed,
                    {"total_requests": len(responses), "status_codes": set(responses), "has_rate_limit": has_429},
                )
                return passed

        except Exception as e:
            self.add_result("白名单路径免限流", False, {"error": str(e)})
            return False
    ...
    async def test_config_snapshot(self) -> bool:
        ...
        for file_path in files_to_check:
            ...
            self.add_result(f"配置文件 {file_path}", exists, {"exists": exists})
